...
const CreateRequest: React.FC<CreateRequestProps> = ({
  ...
  const handleProjectChange = (projectId: string) => {
    ...
    if (selectedProject) {
      const userMember = selectedProject.project_members.find(
              ...
              {projectInfo._id ? (
                ...
                >
                  ...
                  >
                    ...
                    <span>
                      {projectInfo.project_members.find(
                        (member) => member.user_id === userId
                      )?.project_role || "N/A"}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span>{`${dayjs(projectInfo.project_start_date).format(
                      "DD/MM/YYYY"
                    )} - ${dayjs(projectInfo.project_end_date).format(
                      "DD/MM/YYYY"
                    )}`}</span>
                  </div>
                  <div
              ...
              >
                <Form.Item
                  ...
                  rules={[
                    ...
                    {
                      max: 50,
                      message: "Claim name cannot exceed 50 characters!",
                    },
                  ]}
                >
                ...
                <Form.Item
                  ...
                  name="approval_id"
                  rules={[
                    { required: true, message: "Please select the approval!" },
                  ]}
                >
                  <Select
                    showSearch
                  ...
                  <DatePicker
                    ...
                    disabledDate={(current) => {
                      ...
                      return (
                        current &&
                        (current.isBefore(
                          dayjs(projectInfo.project_start_date),
                          "day"
                        ) ||
                          current.isAfter(
                            dayjs(projectInfo.project_end_date),
                            "day"
                          ))
                      );
                    }}
                  ...
                  <DatePicker
                    ...
                    disabledDate={(current) => {
                      ...
                      return (
                        current &&
                        (current.isBefore(
                          startDate || projectInfo.project_start_date,
                          "day"
                        ) ||
                          current.isAfter(
                            dayjs(projectInfo.project_end_date),
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Please enter total hours worked!",
                    },
                    {
                      validator: async (_, value) => {
                        if (value && Number(value) < 1) {
                          throw new Error(
                            "Total work time must be at least 1 hour!"
                          );
                        }
                      },
                    },
